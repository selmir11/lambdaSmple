...
public class PolicyTableMemberDbValidations {
    ...
    public void MemberMedicalRecordsValidations(String recordType, String memberPrefix) {
        switch (recordType) {
            case "medical":
    ...
    private void dentalRecordsValidations(String memberPrefix) {
        for (PolicyTablesEntity dentalEntity : dentalPolicyEntities) {
            if (dentalEntity.getSubscriber_ind().equals("1")) {
                for(MemberDetails subscriber: subscribers){
                    if(subscriber.getFirstName().equals(dentalEntity.getFirst_name())&& dentalEntity.getFirst_name().contains(memberPrefix)){
                ...
                validateDependentDenDetails(dentalEntity, memberPrefix);
            }
        }
    }
    ...
    private void denValidationsCommonForAllMembers(String name, PolicyTablesEntity policyTablesEntity) {
            ...
            softAssert.assertEquals(policyTablesEntity.getPolicy_start_date(), SharedData.getExpectedCalculatedDates_dentalPlan().getPolicyStartDate(), "Dental, Policy start date does not match");
            softAssert.assertEquals(policyTablesEntity.getPolicy_end_date(), SharedData.getExpectedCalculatedDates_dentalPlan().getPolicyEndDate(), "Dental, Policy end date does not match");
            ...
            softAssert.assertEquals(policyTablesEntity.getMember_financial_start_date(), SharedData.getExpectedCalculatedDates_dentalPlan().getMemberFinancialStartDate(), "Dental member financial start date does not match");
            softAssert.assertEquals(policyTablesEntity.getMember_financial_end_date(), SharedData.getExpectedCalculatedDates_dentalPlan().getMemberFinancialEndDate(), "Dental member financial end date does not match");
            softAssert.assertAll();
    }
    private void validateDependentDenDetails(PolicyTablesEntity denEntity, String memPrefix) {
        if (denEntity.getFirst_name().contains(memPrefix)) {
        List<MemberDetails> members = basicActions.getAllDependents();
        for (MemberDetails member : members) {
            if (member.getFirstName().contains(memPrefix)) {
                System.out.println("Den Member:: " + memPrefix);
    ...
    private void valMemMedicalRecordsValidations(String memberPrefix) {
        for (PolicyTablesEntity policyTablesEntity : medicalPolicyEntities) {
            if (policyTablesEntity.getSubscriber_ind().equals("1") && policyTablesEntity.getFirst_name().contains(memberPrefix)) {
                ...
                validateDependentMedDetails(policyTablesEntity, memberPrefix);
        }
    }
    ...
    private void validateSubscriberMedDetails(MemberDetails subscriber, PolicyTablesEntity policyTablesEntity) {
        ...
        softAssert.assertEquals(String.valueOf(policyTablesEntity.getPremium_reduction_type_emcfh()), Integer.parseInt(subscriber.getMedicalAptcAmt()) > 0 ? "APTC" : "null", "Subscriber Medical Policy premium reduction type does not match");
        softAssert.assertEquals(String.valueOf(policyTablesEntity.getPremium_reduction_type_epfh()), Integer.parseInt(subscriber.getMedicalAptcAmt()) > 0 ? "APTC" : "null", "premium reduction type in en policy financial ah table does not match");
    ...
    private void medValidationsCommonForAllMembers(String name, PolicyTablesEntity policyTablesEntity) {
        ...
        System.out.println("Name:: " + policyTablesEntity.getFirst_name());
            ...
            softAssert.assertEquals(policyTablesEntity.getMember_financial_start_date(), SharedData.getExpectedCalculatedDates_medicalPlan().getMemberFinancialStartDate(),"Medical member financial start date does not match");
            softAssert.assertEquals(policyTablesEntity.getMember_financial_end_date(), SharedData.getExpectedCalculatedDates_medicalPlan().getMemberFinancialEndDate(),"Medical member financial end date does not match");
            softAssert.assertAll();
    }

    private void validateDependentMedDetails(PolicyTablesEntity policyTablesEntity, String memberPrefix) {
        if (policyTablesEntity.getFirst_name().contains(memberPrefix)) {
            List<MemberDetails> members = basicActions.getAllDependents();
            for (MemberDetails member : members) {
                if (member.getFirstName().contains(memberPrefix)) {
