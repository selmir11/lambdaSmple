Feature: Open Enrollment Ended Warning Modal related tests
  ...
  @SLCR-693
  Scenario: Open Enrollment Period Ended pop up validation in English when there is no LCE
  ...
  @SLCR-694
  Scenario: Open Enrollment Period Ended pop up validation in Spanish when there is no LCE in spanish
    And I check "None of these" life change event checkbox
    And I select continue on the LCE page
    Then I validate I am on the "CoCo Declarations and Signature" page
    And I enter a valid signature
    ...
    And I click Continue on the Application Results Page CoCo
    Then I validate open enrollment ended pop up in "Spanish"
    And I click close on open enrollment ended pop up modal
    And I click on Sign Out in the Header for "Elmo"
