...
Feature: Connect for Health Colorado connection to PEAK redirection URL validation
  ...
  Scenario: Connect for Health Colorado connection to PEAK redirection URL validation
    ...
    Then I validate the current PEAK connection URL is "https://peak--peaksit02.sandbox.my.site.com/peak/s/user-login?language=en_US&spInitSso=Y&sp=c4hco"
    ...
    And I wait for 2000 milliseconds
    Then I click Ok on the PEAK alert
    Then I validate the current PEAK connection URL is "https://peak--peaksit02.sandbox.my.site.com/peak/s/user-login?language=es&spInitSso=Y&sp=c4hco"
    #These are the correct URLs per PORT-7316 as of December 2024