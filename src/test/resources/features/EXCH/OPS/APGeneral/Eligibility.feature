...
Feature: Manage Plans:Current Dent Container verification (order)
  ...
  Scenario:Admin Portal dashboard Eligibility container with 2 households
    ...
    Then I enter an accountId in any Env "7746317007" "1106017043" to search user
    ...
    And I check "Uncle Charles" "UNCLE" "SNGL" or "BIRTH_ADOPTION_OR_PLACEMENT_FOR_ADOPTION" "2021-04-17" ""
...
@SLER-709
  ...
  Scenario:Admin Portal dashboard Eligibility container past year 2023
    ...
    Then I login as Admin User any environment "adminPortalADUser_UN_STG" password "adminPortalADUser_PW_STG" and "adminPortalADUser_UN_QA" password "adminPortalADUser_PW_QA"
    And I validate I am on the "Admin Portal search" page
    Then I enter an accountId in any Env "4007800744" "7634740008" to search user
    And I click on Search button in AP dashboard page
    And I click acct first row from Search Results table in admin portal
    And I validate I am on the "Admin Portal individual dashboard" page
    Then I verify Primary account holder and account number displays
    ...
    And I check "Member level eligibility data" columns "Member" "CSR" "Relationship to the Primary Account Holder" "Tax Filer Status"
    And I check "Gertrude Simmons" "SELF" "HOH" or "BIRTH_ADOPTION_OR_PLACEMENT_FOR_ADOPTION" "2021-04-17" ""
    And I check "Life change events" columns "Life Change Event" "Impacted Members" "Date of Event"
